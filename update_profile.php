...
// Ambil data lama supaya tidak kosong
$old = $conn->query("SELECT foto, foto_ktm FROM profiles WHERE email='$email'")->fetch_assoc();
if (empty($foto)) $foto = $old['foto'];
if (empty($foto_ktm)) $foto_ktm = $old['foto_ktm'];

$stmt = $conn->prepare("UPDATE profiles SET nama_lengkap=?, alamat=?, no_telepon=?, nama_kampus=?, jurusan=?, semester=?, foto=?, foto_ktm=? WHERE email=?");
$stmt->bind_param("sssssssss", $nama, $alamat, $no_telepon, $kampus, $jurusan, $semester, $foto, $foto_ktm, $email);
